...
two_sum <- function(nums_vector,target){
  ...
  vector_length = length(nums_vector)
  
  for (i in 1:vector_length) {
    for (j in 2:vector_length) {
      if (nums_vector[i] + nums_vector[j] == target) {
        print(c(nums_vector[i],nums_vector[j]))
      }
    }
  }
...
two_sum = function(nums_vector, target) {
  ...
  for (num1 in 1:length(nums_vector)) {
    first_number = nums_vector[num1]
    
    complement = target - first_number

    if (has.key(as.character(complement), y)) {
      values <- append(values, list(c(complement, first_number)))
    }

    y[[as.character(first_number)]] = first_number
  }
